...
function web_scripts() {
	...
	wp_style_add_data( 'web-style', 'rtl', 'replace' );

	wp_enqueue_script( 'web-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
...
function register_post_types(){
	...
	register_post_type( 'tarif', [
		...
		'labels' => [
			...
			'add_new_item'       => 'Добавление тарифов', // заголовка у вновь создаваемой записи в админ-панели.
			'edit_item'          => 'Редактирование тарифов', // для редактирования типа записи
			...
			'not_found_in_trash' => 'Не найдено в корзине тариф', // если не было найдено в корзине
...
//отправка формы 2 часть
// Здесь мы регистрируем нашу функцию «send_form» для обработки нашего AJAX-запроса,
add_action('wp_ajax_send_form', 'send_form'); // Это для авторизованных пользователей
add_action('wp_ajax_nopriv_send_form', 'send_form'); // Это для неавторизованных пользователей.
/**
 * В этой функции мы будем обрабатывать входные данные.
 *
 * @return void
 */
function send_form(){ 
    // Это безопасный процесс проверки того, исходит ли этот запрос из действительного источника.
    check_ajax_referer( 'secure-nonce-name', 'security' );
 
    /**
    Сначала мы делаем некоторые проверки
     */
     
    if ( empty( $_POST["name"] ) ) {
        echo "<div><p>Вставьте имя, пожалуйста</div></p>";
        wp_die();
    }
    if ( empty( $_POST["tel"] ) ) {
        echo "<div><p>Вставьте телефон, пожалуйста</div></p>";
        wp_die();
    }
 
    // Это электронная почта, на которую вы хотите отправить комментарии.
    $to = 'test@yandex.ru';
 
    // Тема вашего сообщения.

    $subject = 'Cообщение от клиента!';
    
    $body  = 'От: ' . $_POST['name'] . '\n';
    $body .= 'Телефон: ' . $_POST['tel'] . '\n';
 
    // Это заголовки сообщений.

    // больше здесь https://developer.wordpress.org/reference/functions/wp_mail/
    $headers = array('Content-Type: text/html; charset=UTF-8');
     
    wp_mail( $to, $subject, $body, $headers );
 
    echo '<div style="color:green"><p>Отправлено!</div></p>';
    wp_die();
}
